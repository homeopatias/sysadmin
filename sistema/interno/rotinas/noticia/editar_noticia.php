...
if(isset($_SESSION["usuario"]) && unserialize($_SESSION["usuario"]) instanceof Administrador
   && unserialize($_SESSION["usuario"])->getNivelAdmin() === "administrador"){
    ...
    if(isset($_POST["submit"])){
        ...
        // se todos os dados estão válidos, a notícia é editada
        if($idValido && $tituloValido && $autorValido && $conteudoValido){
            ...
            $comando .= "WHERE idArtigo = ? AND tipo = 'noticia'";
            ...
            if($sucesso){
                ...
                $mensagem = "Erro na edição de notícia";
            }
...
header('Location: ../../gerenciar_noticias.php'.$mensagem, true, "302");
die();