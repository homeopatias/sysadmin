...
<html>
    <head>
        ...
        <title>Suas aulas desse ano - Homeopatias.com</title>
        ...
        <?php
            ...
            if(isset($_SESSION["usuario"]) && unserialize($_SESSION["usuario"]) instanceof Administrador
               && unserialize($_SESSION["usuario"])->getNivelAdmin() === "professor"){
                ...
                $textoQuery  = "SELECT idAula, chaveCidade, etapa, data, nota,
                                descricao, COUNT(F.chaveAula) as numAval FROM Aula
                                LEFT JOIN  Frequencia F ON F.chaveAula = idAula
                                AND F.jaAvaliou = 1 WHERE idProfessor = :idProfessor
                                GROUP BY idAula";
                
                // Se algum filtro foi repassado, altera o query para filtrar
                $filtroEtapa = $filtroDataMin = $filtroDataMax = false;
                $filtroCidade = false;
                if(isset($_GET["filtro-etapa"]) || isset($_GET["filtro-data-min"]) ||
                   isset($_GET["filtro-data-max"]) || isset($_GET["filtro-cidade"])){
                    ...
                    if(isset($filtroCidade) && mb_strlen($filtroCidade) > 0 && 
                        ...
                        $textoQuery .= " AND chaveCidade = :filtrocidade ";                       
                    }  
                    if(isset($filtroEtapa) && mb_strlen($filtroEtapa) > 0 && 
                        !is_nan($filtroEtapa) && $filtroEtapa != "0"){
                        $textoQuery .= " AND etapa LIKE :filtroetapa ";
                    }
                    if(isset($filtroDataMin) && mb_strlen($filtroDataMin) > 0){
                        $textoQuery .= " AND CAST(data AS Date) >= CAST(:dataMin as Date)";
                    }
                    if(isset($filtroDataMax) && mb_strlen($filtroDataMax) > 0){
                        $textoQuery .= " AND CAST(data AS Date) <= CAST(:dataMax as Date)";
                    }
                }
                ...
                if( isset($_GET["numeroTableHeader"]) ){
                    ...
                    if( !is_nan($indexHeader) ){
                        ...
                        switch ($indexHeader) {
                            case '0':
                                $orderBy = " ORDER BY idAula " ;
                                break;
                            case '1':
                                $orderBy = " ORDER BY chaveCidade " ;
                                break;
                            case '2':
                ...
                $query->bindParam(":idProfessor", 
                                  unserialize($_SESSION["usuario"])->getIdAdmin());
        ...
        <div class="col-sm-12">
            <div class="center-block col-sm-12 no-float">
                <section class="conteudo">
                    ...
                    <form method="GET" action="aulas_professor.php" id="form-filtro">
                            ...
                            <table class="table table-bordered table-striped" id="aulas">
                                <thead style="background-color: #AAA">
                                    <tr>
                                        ...
                                        <th width="50px" <?= $indexHeader == 4 ? 
                                            ($direcao == 1? "class =\"headerSortUp\"" : 
                                                "class =\"headerSortDown\"") : "" ?>>Nota</th>
                                        <th width="100px" <?= $indexHeader == 5 ? 
                                            ($direcao == 1? "class =\"headerSortUp\"" : 
                                                "class =\"headerSortDown\"") : "" ?>>Número de avaliações</th>
